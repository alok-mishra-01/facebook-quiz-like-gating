...
//---- EVENT HANDLERS ------------------------------------------------------------------------------------
		...
		/** @private **/
		protected function _httptatusHandler(event:Event):void {
			_httptatus = (event as Object).status;
...
//---- GETTERS / SETTERS -------------------------------------------------------------------------
		...
		/** The httptatus code of the loader. You may listen for <code>LoaderEvent.HTTP_STATUS</code> events on certain types of loaders to be notified when it changes, but in some environments the Flash player cannot sense httptatus codes in which case the value will remain <code>0</code>. **/
		public function get httptatus():int {
			return _httptatus;
		}
